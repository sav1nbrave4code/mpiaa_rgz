...
/**
 * Non-oriented weighted scene_graph.
**/
class Graph {
public:
    Graph() {}
    ...
    /// Add single vertex to the scene_graph.
    void add_vertex(int vertex);

    /// Add vertices (if necessary) and an edge between them to the scene_graph.
    void add_edge(int start_vertex, int end_vertex, double weight = 1.0);

    /// Return all vertices of the scene_graph.
    ...
    /// Return true if the vertex exists in the scene_graph, false otherwise.
    ...
    /// Remove the vertex and all its adjacent edges from the scene_graph.
    void remove_vertex(int vertex);

    /// Remove the edge from the scene_graph (but not the vertices).
